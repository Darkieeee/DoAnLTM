...
public class Calender {
    ...
    private int getFloor(ArrayList<CTMonhoc> ctmh) {
        ...
        for (int i=0; i<ctmh.size();i++) {
            try {
                if (Integer.parseInt(ctmh.get(i).maphong.substring(3,4))>max) {
                    max = Integer.parseInt(ctmh.get(i).maphong.substring(3,4));
                }
            }
            catch (Exception e) {
                if (Integer.parseInt(ctmh.get(i).maphong.substring(4,5))>max) {
                    max = Integer.parseInt(ctmh.get(i).maphong.substring(4,5));
                }
            }
        }
